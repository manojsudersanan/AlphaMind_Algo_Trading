...
class VirtualBroker:
    ...
    def execute_market_order(self, symbol: str, quantity: int, current_market_price: float) -> dict:
        is_buy = quantity > 0
        ...
        print(f"[VirtualBroker] Simulated {'BUY' if is_buy else 'SELL'} {abs(quantity)} {symbol} @ {fill_price:.2f} (Comm: ₹{commission:.2f})")
